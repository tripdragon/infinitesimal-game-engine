...
import { init as initParticlesSystem, genParticles } from './particles-on-click.js';
...
export const initState = ({ shouldConnectToNetwork = false, wsUrl } = {}) => {
  ...
  const sessionId = shouldConnectToNetwork ? (storageState._sessionId || previousSessionId || uuid()) : uuid();
  ...
  if (!cache._isStateInitialized) {
    if (isNaN(storageVersion) || (!isNaN(storageVersion) && storageVersion < defaultVersion)) {
      internals.saveState(merge(defaultState, { _sessionId: sessionId, _previousSessionIds: [] }));
    } else {
      internals.saveState(
        merge(cache, {
...
internals.saveState = data => {
  ...
  const storageObj = JSON.stringify({ ...cache, _version: String(cache._version) });
...
const handleSyncRequest = ({ state, data }) => {
  ...
  const stateWithPublicVars = Object.entries(state).reduce((obj, [key, val]) => {
    if (key.startsWith("_")) {
      return obj;
    }
    return {
  ...
  return sendNetworkedEvent({ to: data._senderId, stateEvent: { eventName: "syncResponse", ...stateWithPublicVars } });
};
...
internals.abbreviateUserId = (userId) => `...${(userId || '').split('-').pop()}`;
